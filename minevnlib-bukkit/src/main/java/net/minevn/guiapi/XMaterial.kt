...
enum class XMaterial(
     ...
     */
    BLACK_BED(if (supports(12)) 15 else 0, "BED_BLOCK", "BED"),
    BLACK_CANDLE,
    BLACK_CANDLE_CAKE,
    ...
    BLUE_BED(if (supports(12)) 11 else 0, "BED_BLOCK", "BED"),
    BLUE_CANDLE,
    BLUE_CANDLE_CAKE,
    ...
    BROWN_BED(if (supports(12)) 12 else 0, "BED_BLOCK", "BED"),
    BROWN_CANDLE,
    BROWN_CANDLE_CAKE,
    ...
    /**
     * 1.13 tag is not added because it's the same thing as [.AIR]
     *
     * @see .VOID_AIR
     */
    CAVE_AIR("AIR"),
    ...
    CYAN_BED(if (supports(12)) 9 else 0, "BED_BLOCK", "BED"),
    CYAN_CANDLE,
    CYAN_CANDLE_CAKE,
     ...
     * [GitHub Issue](https://github.com/CryptoMorin/XSeries/issues/91)
     */
    FILLED_MAP("MAP"),
    FIRE,
    ...
    GRAY_BED(if (supports(12)) 7 else 0, "BED_BLOCK", "BED"),
    GRAY_CANDLE,
    GRAY_CANDLE_CAKE,
    ...
    GREEN_BED(if (supports(12)) 13 else 0, "BED_BLOCK", "BED"),
    GREEN_CANDLE,
    GREEN_CANDLE_CAKE,
    ...
    LIGHT_BLUE_BED(if (supports(12)) 3 else 0, "BED_BLOCK", "BED"),
    LIGHT_BLUE_CANDLE,
    ...
    LIGHT_GRAY_BED(if (supports(12)) 8 else 0, "BED_BLOCK", "BED"),
    LIGHT_GRAY_CANDLE,
    ...
    LIME_BED(if (supports(12)) 5 else 0, "BED_BLOCK", "BED"),
    LIME_CANDLE,
    LIME_CANDLE_CAKE,
    ...
    MAGENTA_BED(if (supports(12)) 2 else 0, "BED_BLOCK", "BED"),
    MAGENTA_CANDLE,
    MAGENTA_CANDLE_CAKE,
    ...
    /**
     * Just like mentioned in [Nether Wart](https://minecraft.wiki/w/Nether_Wart)
    ...
    ORANGE_BED(if (supports(12)) 1 else 0, "BED_BLOCK", "BED"),
    ORANGE_CANDLE,
    ORANGE_CANDLE_CAKE,
    ...
    PINK_BED(if (supports(12)) 6 else 0, "BED_BLOCK", "BED"),
    PINK_CANDLE,
    PINK_CANDLE_CAKE,
    ...
    PURPLE_BED(if (supports(12)) 10 else 0, "BED_BLOCK", "BED"),
    PURPLE_CANDLE,
    PURPLE_CANDLE_CAKE,
     ...
     * Data value 14 or 0
     */
    RED_BED(if (supports(12)) 14 else 0, "BED_BLOCK", "BED"),
    RED_CANDLE,
    RED_CANDLE_CAKE,
     ...
     * [Levelled](https://hub.spigotmc.org/javadocs/spigot/org/bukkit/block/data/Levelled.html) water flowing system.
     */
    ...
    YELLOW_BED(if (supports(12)) 4 else 0, "BED_BLOCK", "BED"),
    YELLOW_CANDLE,
    YELLOW_CANDLE_CAKE,
    ...
    /**
     * The data value of this material [pre-flattening](https://minecraft.wiki/w/Java_Edition_data_values/Pre-flattening).
     *
     *
     * Can be accessed with [ItemStack.getData] then `MaterialData#getData()`
     * or [ItemStack.getDurability] if not damageable.
     *
    ...
    init {
        ...
        if ((!Data.ISFLAT && this.isDuplicated) || (Material.getMaterial(
                this.name
            ).also { mat = it }) == null
        ) {
            for (i in legacy.indices.reversed()) {
    ...
    constructor(vararg legacy: String) : this(0, *legacy)

    @Deprecated("Use XTag.stringMatcher() instead.")
    fun isOneOf(materials: Collection<String>?): Boolean {
        if (materials == null || materials.isEmpty()) return false
        val name = this.name

        for (m in materials) {
            var comp = m
            val checker = comp.uppercase()
            ...
            if (checker.startsWith("REGEX:")) {
                ...
                var pattern = CACHED_REGEX.getIfPresent(comp)
                if (pattern == null) {
                    try {
    ...
    /**
     * Sets the [Material] (and data value on older versions) of an item.
     * Damageable materials will not have their durability changed.
     *
     *
     * Use [.parseItem] instead when creating new ItemStacks.
     *
     ...
     * @since 2.0.0
     */
    private fun anyMatchLegacy(@Nonnull name: String): Boolean {
        for (i in legacy.indices.reversed()) {
            if (name == legacy[i]) return true
        }
        return false
    }
    ...
    override fun toString(): String {
        return Arrays.stream(name.split("_".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray())
            .map { t: String -> t[0].toString() + t.substring(1).lowercase(Locale.getDefault()) }
    ...
    val id: Int
         ...
         * [ID List](https://www.minecraftinfo.com/idlist.htm)
         *
         *
        ...
        get() {
            ...
            val material = this.parseMaterial() ?: return -1
            return try {
                material.id
     ...
     */
    @Suppress("deprecation")
    fun parseItem(): ItemStack? {
        val material = this.parseMaterial() ?: return null
        val base = if (Data.ISFLAT) ItemStack(material) else ItemStack(
            material, 1,
            data.toShort()
        )
        // Splash Potions weren't an official material pre-flattening.
        if (!Data.ISFLAT && this == SPLASH_POTION) {
            base.durability = 16384.toShort() // Hard-coded as 'data' is only a byte.
        }
        return base
    }
     ...
     */
    @Suppress("deprecation")
    fun isSimilar(@Nonnull item: ItemStack): Boolean {
        Objects.requireNonNull(item, "Cannot compare with null ItemStack")
        if (item.type != this.parseMaterial()) return false
        // Special case for splash potions.
        if (this == SPLASH_POTION) {
            return Data.ISFLAT || item.durability == 16384.toShort()
        }
        return Data.ISFLAT || item.durability == data.toShort() || item.type.maxDurability > 0
    }
    ...
    val isSupported: Boolean
         ...
         *
         *
         * Note that you should use [.parseMaterial] or [.parseItem] and check if it's null
         * if you're going to parse and use the material/item later.
         *
         * @return true if the material exists in [Material] list.
         * @since 2.0.0
         */
        get() = this.material != null
     ...
     * @return this material or the `alternateMaterial` if not supported.
     */
    fun or(alternateMaterial: XMaterial?): XMaterial? {
        return if (isSupported) this else alternateMaterial
    }

    private val isDuplicated: Boolean
        /**
         ...
         *
         *
         * This method should not be called if the version is after the flattening update [Data.ISFLAT]
         * and is only used for parsing materials, not matching, for matching check [.DUPLICATED]
         */
        get() = when (this.name) {
            "MELON", "CARROT", "POTATO", "GRASS", "BRICK", "NETHER_BRICK", "DARK_OAK_DOOR", "ACACIA_DOOR", "BIRCH_DOOR", "JUNGLE_DOOR", "SPRUCE_DOOR", "MAP", "CAULDRON", "BREWING_STAND", "FLOWER_POT" -> true
            else -> false
        }
    ...
    public object Data {
        ...
        init { // This needs to be right below VERSION because of initialization order.
            ...
            val matcher = Pattern.compile("MC: \\d\\.(\\d+)").matcher(version)

            if (matcher.find()) VERSION = matcher.group(1).toInt()
            else throw IllegalArgumentException("Failed to parse server version from: $version")
        }
         ...
         * @since 3.0.0
         */
        val ISFLAT: Boolean = supports(13)
    }

    companion object {
        /**
         * Cached array of [XMaterial.values] to avoid allocating memory for
         * calling the method every time.
         *
         * @since 2.0.0
         */
        val VALUES: Array<XMaterial> = entries.toTypedArray()

        /**
         * We don't want to use [Enums.getIfPresent] to avoid a few checks.
         *
         * @since 5.1.0
         */
        private val NAMES: MutableMap<String, XMaterial> = HashMap()
         ...
         * @since 1.0.0
         */
        private val NAME_CACHE: Cache<String, XMaterial> = CacheBuilder.newBuilder()
            .expireAfterAccess(1, TimeUnit.HOURS)
            .build()

        /**
         * This is used for [.isOneOf]
         *
         * @since 3.4.0
         */
        private val CACHED_REGEX: Cache<String, Pattern?> = CacheBuilder.newBuilder()
            .expireAfterAccess(3, TimeUnit.HOURS)
            .build()

        /**
         * The maximum data value in the pre-flattening update which belongs to [.VILLAGER_SPAWN_EGG]<br></br>
         * [Spawn Eggs](https://minecraftitemids.com/types/spawn-egg)
         *
         * @see .matchXMaterialWithData
         * @since 8.0.0
         */
        private const val MAX_DATA_VALUE: Byte = 120
         ...
         * @since 8.0.0
         */
        private const val UNKNOWN_DATA_VALUE: Byte = -1

        /**
         * The maximum material ID before the pre-flattening update which belongs to [.MUSIC_DISC_WAIT]
         *
         * @since 8.1.0
         */
        private const val MAX_ID: Short = 2267
         ...
         *
         *
         * Duplications are not useful at all in versions above the flattening update [Data.ISFLAT]
         * This set is only used for matching materials, for parsing refer to [.isDuplicated]
         *
         * @since 3.0.0
         */
        private var DUPLICATED: HashSet<String>? = null

        init {
            for (material in VALUES) NAMES[material.name] = material
        }
        ...
        /**
         * Gets the XMaterial with this name similar to [.valueOf]
         ...
         */
        @Nonnull
        private fun getIfPresent(@Nonnull name: String): Optional<XMaterial> {
            return Optional.ofNullable(NAMES[name])
        }
         ...
         * @since 1.0.0
         */
        private fun requestOldXMaterial(@Nonnull name: String, data: Byte): XMaterial? {
            val holder = name + data
            val cache = NAME_CACHE.getIfPresent(holder)
         ...
         * The separator for the material name and its data value is `:`
         ...
         */
        @Nonnull
        private fun matchXMaterialWithData(@Nonnull name: String): Optional<XMaterial> {
            val index = name.indexOf(':')
            if (index != -1) {
                val mat = format(name.substring(0, index))
                try {
                    // We don't use Byte.parseByte because we have our own range check.
                    val data = name.substring(index + 1).replace(" ", "").toInt().toByte()
                    return if (data >= 0 && data < MAX_DATA_VALUE) matchDefinedXMaterial(
                        mat,
                        data
                    ) else matchDefinedXMaterial(mat, UNKNOWN_DATA_VALUE)
         ...
         * value in the string if attached. Check [.matchXMaterialWithData] for more info.
         *
         * @see .matchXMaterialWithData
         * @see .matchDefinedXMaterial
         * @since 2.0.0
         */
        @Nonnull
        fun matchXMaterial(@Nonnull name: String?): Optional<XMaterial> {
            require(!(name == null || name.isEmpty())) { "Cannot match a material with null or empty material name" }
            val oldMatch = matchXMaterialWithData(name)
            return if (oldMatch.isPresent) oldMatch else matchDefinedXMaterial(format(name), UNKNOWN_DATA_VALUE)
        }
        ...
        @Nonnull
        @Suppress("deprecation")
        fun matchXMaterial(@Nonnull item: ItemStack): XMaterial {
            Objects.requireNonNull(item, "Cannot match null ItemStack")
            var material = item.type.name
            ...
            val data =
                (if (Data.ISFLAT || material == "MAP" || item.type.maxDurability > 0) 0 else item.durability).toByte()

            // Versions 1.9-1.12 didn't really use the items data value.
            if (supports(9) && !supports(13) && item.hasItemMeta() && material == "MONSTER_EGG") {
                val meta = item.itemMeta
                if (meta is SpawnEggMeta) {
                    material = meta.spawnedType.name + "_SPAWN_EGG"
                }
            }
            ...
            if (!supports(9) && material.endsWith("ION")) {
                ...
                return if (Potion.fromItemStack(item).isSplash) SPLASH_POTION else POTION
            }
            ...
            if (supports(13) && !supports(14)) {
                ...
                if (material == "CACTUS_GREEN") return GREEN_DYE
                if (material == "ROSE_RED") return RED_DYE
                if (material == "DANDELION_YELLOW") return YELLOW_DYE
            }
            ...
            val result = matchDefinedXMaterial(material, data)
            if (result.isPresent) return result.get()
            throw IllegalArgumentException("Unsupported material from item: $material ($data)")
        }

        /**
         * Gets the XMaterial based on the material's ID (Magic Value) and data value.<br></br>
        ...
        @Nonnull
        @Deprecated(
            """this method loops through all the available materials and matches their ID using {@link #getId()}
      ...
      This method obviously doesn't work for 1.13+ and will not be supported. This is only here for debugging purposes."""
        ...
        fun matchXMaterial(id: Int, data: Byte): Optional<XMaterial> {
            if (id < 0 || id > MAX_ID || data < 0) return Optional.empty()
            for (materials in VALUES) {
                if (materials.data == data && materials.id == id) return Optional.of(materials)
            }
            return Optional.empty()
        }
         ...
         * @param data the data value of the material. Is always 0 or [.UNKNOWN_DATA_VALUE] when [Data.ISFLAT]
         ...
         */
        @Nonnull
        protected fun matchDefinedXMaterial(@Nonnull name: String, data: Byte): Optional<XMaterial> {
            ...
            val isAMap = name.equals("MAP", ignoreCase = true)

            // Do basic number and boolean checks before accessing more complex enum stuff.
            if (Data.ISFLAT || (!isAMap && data <= 0 && !(isDuplicated(name).also { duplicated = it }))) {
                val xMaterial = getIfPresent(name)
                if (xMaterial.isPresent) return xMaterial
            }

            // Usually flat versions wouldn't pass this point, but some special materials do.
            val oldXMaterial = requestOldXMaterial(name, data)
                ?: // Special case. Refer to FILLED_MAP for more info.
                return if ((data >= 0 && isAMap)) Optional.of(FILLED_MAP) else Optional.empty()
             ...
             * Plural methods are only plural if they're also [.DUPLICATED]
             *
             * The only special exceptions are [.BRICKS] (??) and [.NETHER_BRICKS]
             * Note: BRICKS was added because
             * `XMaterial.matchXMaterial("BRICK")` would match [.BRICKS] instead in 1.8.
             */
            val isPlural = oldXMaterial == CARROTS || oldXMaterial == POTATOES || oldXMaterial == BRICKS

            if (!Data.ISFLAT && isPlural && (if (duplicated == null) isDuplicated(name) else duplicated!!)) return getIfPresent(
                name
            )
         ...
         */
        @Nonnull
        protected fun format(@Nonnull name: String): String {
            val len = name.length
            val chs = CharArray(len)
            var count = 0
            var appendUnderline = false

            for (i in 0 until len) {
                val ch = name[i]

                if (!appendUnderline && count != 0 && (ch == '-' || ch == ' ' || ch == '_') && chs[count] != '_') appendUnderline =
                    true
                else {
                    var number = false
                    // Old materials have numbers in them.
                    if ((ch >= 'A' && ch <= 'Z') || (ch >= 'a' && ch <= 'z') || ((ch >= '0' && ch <= '9').also {
                            number = it
                        })) {
                        if (appendUnderline) {
                            chs[count++] = '_'
                        ...
                        else chs[count++] = (ch.code and 0x5f).toChar()
                    }
                }
            }
         ...
         * **Example:**
         *
         * `MELON, CARROT, POTATO, BEETROOT -> true`
         *
         ...
         * @since 2.0.0
         */
        private fun isDuplicated(@Nonnull name: String): Boolean {
